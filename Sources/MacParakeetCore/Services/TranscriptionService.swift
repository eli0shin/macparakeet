    ...
    MeetingSpeakerAttributionCorrectingTranscriptionService
{
    ...
    public func correctMeetingSpeakerAttribution(
        ...
        let constraint = try selection.remoteDiarizationConstraint(
            hasSystemAudio: recording.sourceAlignment.system != nil)
        guard let words = original.wordTimestamps, !words.isEmpty else {
            throw MeetingSpeakerCountCorrectionError.timedWordsUnavailable
        }
        guard let diarizationService, let systemTrack = recording.sourceAlignment.system else {
        ...
        let systemDiarization = mappedMeetingSystemDiarization(
            diarResult, systemStartOffsetMs: systemTrack.startOffsetMs)
        ...
        do {
            ...
            logger.error(
                "Speaker correction committed but derived search refresh failed error_type=\(DiagnosticErrorClassifier.classify(error), privacy: .public)"
            )
        }
    ...
    private func transcribeMeetingAudio(
        ...
        let diarizationRequested =
            diarizationService != nil && shouldDiarizeMeetings() && recording.sourceAlignment.system != nil
    ...
    private func assertCanTranscribe() async throws {
        do {
            if let entitlements {
