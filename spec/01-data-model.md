...
- New finalized and explicitly re-transcribed meetings preserve untouched STT text in `rawTranscript` and store a separate deterministic readable projection in `cleanTranscript`. Meeting cleanup removes only shared always-safe fillers, applies enabled custom words, normalizes whitespace and punctuation, and capitalizes sentence starts. It does not expand snippets, execute actions, apply paste styling, or use an LLM. Existing meeting rows are not backfilled; consumers derive the same cleaned Reading Turn fallback in memory when `cleanTranscript` is `NULL`. Raw words, timing, source attribution, and diarization remain canonical evidence.
