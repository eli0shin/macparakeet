...
The default completed-meeting transcript uses the selected Conversation prototype layout. All transcript sizes use a plain stack to avoid the macOS 26 lazy-layout loop with selectable, variable-height text. Row count must not select the faulty lazy path; a future bounded layout needs to pass the same scrolling correctness and frame-latency gates before use. The stack renders one card per Reading Turn, never one view per word. Each card presents one speaker header and one secondary start-time button. Long turns keep several selectable paragraphs inside the same card. Paragraph breaks follow complete sentences within the three-sentence and 80-word bounds, or a 2.5-second pause, without repeating the card header. Readable wording uses conservative local cleanup while all source-word references remain available as evidence. When the existing transcript AI formatter is enabled, a card can show a validated formatted text override keyed to that Reading Turn and its deterministic source text; failed, invalid, stale, or incomplete overrides show the deterministic wording without changing the card identity or timing. The start-time button supports pointer and keyboard activation, seeks to the turn start, and starts playback. Playback focus strengthens the active card border. Transcript find searches complete turns in reading order and scrolls to the containing card. VoiceOver encounters the speaker group, optional start time, and text in that order. Simultaneous contributions appear inside one labelled overlap container in deterministic reading order; each contribution keeps its own speaker card and seek button. A short backchannel can share this container with one continuous surrounding statement instead of splitting that statement into two cards. Untimed fallback cards omit the timestamp instead of implying unavailable precision. Each card has a **Copy Reading Turn** context action that copies its current speaker, one available start time, and readable paragraphs. Full meeting copy, readable exports, and rich AI context use the same document structure. The Text mode remains available for editing the plain transcript.
