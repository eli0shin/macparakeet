...
The default completed-meeting transcript uses the selected Compact byline layout. All transcript sizes use a plain stack to avoid the macOS 26 lazy-layout loop with selectable, variable-height text. Row count must not select the faulty lazy path; a future bounded layout needs to pass the same scrolling correctness and frame-latency gates before use. The stack renders one borderless block per displayed turn, never one view per word. A displayed turn collapses every consecutive Reading Turn with the same capture source and resolved speaker ID. A pause of any length, an internal paragraph or formatting boundary, or overlap metadata does not split that run. Only an intervening different speaker ends it; different speaker IDs remain separate even when their labels match. A restrained speaker dot, speaker name, and quiet start-time button form one compact byline above the run's full-width prose. The block renders exactly one empty text row between paragraphs and retains all internal word references, but only its first Reading Turn identity and start time are UI navigation targets. No internal timestamp, playback-focus row, or accessibility turn is rendered. Readable wording uses conservative local cleanup. Validated AI formatting is combined in order without changing canonical formatting mappings. The start-time button supports pointer and keyboard activation, seeks to the displayed turn start, and starts playback. Playback focus uses only a left border; it adds no gradient or background fill. Transcript find searches displayed turns in reading order and scrolls to the containing block. VoiceOver encounters the speaker group, optional start time, and text in that order. Contributions follow word start time, with original evidence order breaking ties. An interruption splits the surrounding speaker's text so a short question stays before later speech. Live preview uses the same chronological rule. No simultaneous-speech grouping, label, border, background, or wrapper spacing is used; canonical word and diarization evidence remain unchanged. Untimed fallback blocks omit the timestamp instead of implying unavailable precision. Each block has a **Copy Reading Turn** context action that copies its current speaker, one available start time, and readable paragraphs. Full meeting copy, readable exports, and rich AI context keep the canonical Reading Turn document and do not inherit UI grouping. The Text mode remains available for editing the plain transcript.

The completed-meeting speaker overview uses compact inline entries in a wrapping flow. Each entry keeps one restrained identity dot, keyboard-accessible rename control, speaking time, and word count. Entries add another compact line only when the available width cannot contain them. The overview remains collapsible and its disclosure and rename controls keep explicit accessibility labels and keyboard focus behavior.

Completed meetings show the detected total-person count and an **Adjust Speakers** secondary action. The action opens a popover, never a completion modal. Its Auto, Exact, and Range choices define total people including Me. Inline fields reject non-integers, totals below two, and reversed bounds before work starts. While attribution reruns, the popover shows local progress and Cancel; the current transcript remains in place. Inline errors preserve that presentation. A successful rerun closes the popover and refreshes all Reading Turn labels.
...
│  SPEECH ENGINE                                            │
│  ─────────────────────────────────────────────────────    │
│  Your selected engine handles dictation, meetings,        │
...
│    Optionally use a different engine for recordings &     │
│    files.                                                 │
└───────────────────────────────────────────────────────────┘
```
...
- The recordings/files override is progressive disclosure at the bottom of the card. `Advanced transcription` is collapsed by default; its summary says whether recordings and files inherit the selected engine or use an override. This keeps the normal one-engine setup legible without hiding an active split.
