...
The default completed-meeting transcript uses the selected Compact byline layout. All transcript sizes use a plain stack to avoid the macOS 26 lazy-layout loop with selectable, variable-height text. Row count must not select the faulty lazy path; a future bounded layout needs to pass the same scrolling correctness and frame-latency gates before use. The stack renders one borderless block per Reading Turn, never one view per word. A restrained speaker dot, speaker name, and quiet start-time button form a compact byline above full-width prose. Long turns keep several selectable paragraphs inside the same block. Paragraph breaks follow complete sentences within the three-sentence and 80-word bounds, or a 2.5-second pause, without repeating the byline. Readable wording uses conservative local cleanup while all source-word references remain available as evidence. When the existing transcript AI formatter is enabled, a block can show a validated formatted text override keyed to that Reading Turn and its deterministic source text; failed, invalid, stale, or incomplete overrides show the deterministic wording without changing the block identity or timing. The start-time button supports pointer and keyboard activation, seeks to the turn start, and starts playback. Playback focus uses only a left border; it adds no gradient or background fill. Transcript find searches complete turns in reading order and scrolls to the containing block. VoiceOver encounters the speaker group, optional start time, and text in that order. Simultaneous contributions render as ordinary adjacent blocks in deterministic reading order without an overlap label, border, background, or wrapper spacing. Their overlap identity and timing evidence remain unchanged for internal semantics and non-visual consumers. A short backchannel can remain associated with one continuous surrounding statement without splitting that statement into two blocks. Untimed fallback blocks omit the timestamp instead of implying unavailable precision. Each block has a **Copy Reading Turn** context action that copies its current speaker, one available start time, and readable paragraphs. Full meeting copy, readable exports, and rich AI context use the same document structure. The Text mode remains available for editing the plain transcript.
