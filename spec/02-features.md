...
**Scope:** File/URL transcription and completed meetings. Dictation is single-speaker by design.
...
Completed meetings use a pure local presentation builder over the existing source-aware words and retained diarization regions. It forms microphone and system utterances independently so chronological word interleaving does not become one visible card per source change. The live meeting panel's audio-controls popover exposes separate **System audio** and **Microphone** speaker-detection toggles. Each changes only its named track, applies to later live-preview chunks and the active meeting's final transcript, and becomes the default for new meetings; unavailable capture tracks stay visible but disabled. Already displayed live words keep their attribution, while in-flight chunk results use the latest successfully persisted choice before display. Local speaker indices restart for each independent live diarization call, so the preview gives each call a distinct identity namespace instead of treating separate `S1` results as the same person without matching evidence. Microphone turns are labelled **Me** when microphone speaker detection is off. The independent **Detect speakers on microphone** default remains off. When enabled, microphone turns use local diarization evidence and labels **Local Speaker N**, with **Local Speakers** as the unattributed fallback. Local and remote evidence never share speaker identities. System utterances use aggregate diarization overlap to choose an available remote-speaker label or the **Others** fallback. Equal evidence does not receive an arbitrary speaker. An isolated remote-speaker run shorter than one second is absorbed when the same stable speaker surrounds it, while sustained evidence remains a distinct exchange. A short run with at least 200 ms of concurrent remote-speaker evidence remains as a genuine backchannel, and the stable statement around it stays one Reading Turn. Generic fallback words inherit the refined identity of their utterance, and adjacent same-speaker utterances merge across pauses shorter than 2.5 seconds. Clear pauses and completed capture-source exchanges remain visible boundaries.

Microphone/system Reading Turns that overlap by at least 200 ms share an explicit overlap group. Remote Reading Turns share one only when retained diarization regions for both resolved speakers overlap by at least 200 ms. Each contribution keeps its speaker, full text, time range, word references, and seek target. Weak fallback and unattributed fragments do not receive a confident identity for this presentation. Overlap groups use the earliest contribution as their stable identity and preserve deterministic chronological and capture-source order.

Each Reading Turn is a compact borderless block with one restrained speaker dot, a speaker byline, one quiet seekable start timestamp when timing exists, and one or more paragraphs. Playback focus adds only a left border. Overlapping turns appear as ordinary adjacent blocks without a labelled or decorated overlap wrapper; their overlap identity and timing evidence remain unchanged. Paragraphs pack complete sentences to the established three-sentence and 80-word bounds and split at 2.5-second pauses; an indivisible sentence can exceed the word bound. Readable text removes only approved hesitation sounds and overlapping exact repetition, applies custom vocabulary, and normalizes whitespace and punctuation without dictation snippets, paste actions, or insertion styling. Raw processing mode selects the same builder's verbatim wording without changing turn identity, attribution, paragraph structure, order, timing, or evidence references. The original words, word-level labels, source IDs, times, and diarization regions remain canonical and unchanged. Untimed meetings show a useful text-only Reading Turn without a timestamp or precise speaker claim. The prior Text surface remains available for plain-text editing. The speaker overview presents renameable speakers and compact speaking-time and word-count statistics in an inline flow that wraps only when space is insufficient.

Completed meetings also expose a non-blocking **Adjust Speakers** popover. On recordings with microphone speaker detection enabled, users choose the microphone or system track and all constraints count only speakers on that selected track, starting at one. Both tracks keep independent speaker identities, and correction preserves the other track's names and evidence. The following total-including-Me behavior applies to legacy recordings and recordings with microphone detection off. Auto remains the default and normal meeting completion never opens this popover. A user can rerun only system-audio speaker attribution with Auto, an exact total, or lower and upper total bounds. All entered and detected counts mean total people in the meeting, including Me; the service subtracts Me before it passes the constraint to the existing remote-speaker diarizer. A valid constrained count therefore starts at two total people. Microphone-only meetings, invalid bounds, missing timed words, and missing retained system audio receive local feedback before diarization starts. The old transcript remains visible during progress, cancellation, and errors. Success replaces speaker evidence and rebuilds transcript segments and Reading Turns without rerunning speech recognition or changing transcript words. If retranscription replaces the canonical words while a correction runs, the stale correction is rejected without changing the retranscribed meeting.

Completed-meeting copy, readable TXT and Markdown exports, `meeting.md`, and rich summary/chat context project this same Reading Turn document. These projections keep speaker order, explicit overlap grouping, and paragraph boundaries, and emit no more than one start time per turn. A card context action copies one complete Reading Turn with its current speaker and available start time. Word citations resolve to the containing turn and its seekable range. Edited transcript text and plain AI context remain available without invalid word alignment, while SRT and VTT continue to use subtitle cues built from canonical word timing.
