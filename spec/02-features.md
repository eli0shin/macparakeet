...
- formatter runs for dictation, file/URL, and meeting transcription flows — every transcription finalization path shares `completeTranscription`, which invokes the formatter with the applicable `FormatterSource`
...
- `transform_history` stores local input/output/source-app/timing rows for completed Transform runs. This is deliberate local user data. The fork has no Transform telemetry instrumentation.
...
**Still proposed:** Signal amplitude alone does not restart the microphone. A separately proposed offline VAD pass would find transcript gaps and re-transcribe missed speech; the implemented frame report measures recorded media coverage, not transcript completeness.
