...
6. **Complete context is mandatory.** Every LLM operation sends its complete relevant source text, applicable notes, instructions, and chat history through one provider-independent assembly policy. Provider adapters and feature modes cannot truncate, compact, retrieve, rank, select, summarize, chunk, or drop context. A provider context-window rejection is returned clearly and is never followed by a reduced-context retry.
