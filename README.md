...
- **AI formatter** — Optionally run your dictation and file transcripts through your AI provider to clean up grammar, punctuation, and paragraphing. Meeting cleanup can also move clearly misplaced words between neighboring speaker turns when **Fix misplaced words between speakers** is enabled. This changes cleaned presentation text only; raw words, speakers, timestamps, and diarization evidence stay unchanged. Toggle on/off, customize the prompt, or reset to default.
...
- **Meeting AI cleanup diagnostics.** Local `~/Library/Logs/MacParakeet/meeting-ai-cleanup.jsonl` records full cleanup input/output text and exact acceptance or rejection reasons. These records contain transcript content; inspect them before sharing logs. They are not uploaded automatically.
