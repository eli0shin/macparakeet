...
A local run on the affected 129-second system track used the cached model and reused its 347 saved STT words. The intended final settings were not verified at runtime; the dispatch defect below invalidates the earlier claim that this run used them. Through finalization, temporary persistence, reload and export, the result had two speakers and five Reading Turns instead of three labels and 31 turns. All word text and timestamps were unchanged; references remained in original order. However, the model still assigned the second speaker to 26.55–28.81 seconds inside the opening the user identified as one speaker. The acoustic attribution issue is unresolved. This run is not a fresh ASR evaluation or proof that the complete transcript is correct. Audio, transcript output, and the local probe remain outside the repository.

### Final-configuration dispatch correction

The AMI harness exposed a 32,000-sample hop (2 seconds at 16 kHz) despite the intended 0.1 ratio. A reduced check using the production service through `DiarizationServiceProtocol` confirmed that omitting `speakerConstraint` called the protocol extension's fallback and bypassed the final manager. Passing explicit `nil` reached the final manager. The fix replaces the fallback's default argument with a separate no-constraint overload that explicitly forwards through the protocol requirement.

On the same complete ES2011a headset recording, the corrected CLI logged a 160,000-sample window and 16,000-sample hop (1 second). The before/after runs used identical audio/reference snapshot and cached model hashes and completed isolated save, reload, export and scoring. This verifies configuration delivery, not correct speaker attribution. The affected private recording has not been rerun after this dispatch fix. Model choice, embedding defaults and live behavior remain unchanged.
